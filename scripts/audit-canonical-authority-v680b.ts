...
check('2.1 retriever exposes canonicalAuthorities',/canonicalAuthorities\?\s*:\s*Array<string\s*\|\s*CanonicalSeed>/.test(ret));
...
check('3.1 provider-aware bounded network plan preserved',/buildAuthorityProviderPlan\s*\(/.test(ret) && /regulationBudget=6/.test(ret) && /judicialProductBudget=2/.test(ret) && /caseLawBudget=2/.test(ret));
