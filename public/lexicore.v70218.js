...
function syncBusyVisual(percent,mode='indeterminate'){
  const safe=Math.max(0,Math.min(100,Number(percent)||0));
  const bar=document.getElementById('busyBar');
  const ring=document.querySelector('#busyOverlay .ring');
...
function setBusy(on,label='LexiCore sedang memproses'){
 ...
 if(on){busyDepth++;if(busyDepth>1)return;busyStarted=Date.now();resetBusyProgress();busyTitle.textContent=label;busyText.textContent='Menyiapkan proses...';busyElapsed.textContent='0%';syncBusyVisual(0,'indeterminate');overlay.classList.add('show');overlay.setAttribute('aria-hidden','false');chip.classList.add('show');processChipText.textContent=label;document.body.classList.add('is-busy');busyTimer=setInterval(()=>{if(busyProgressMode){busyElapsed.textContent=Math.round(busyProgressValue)+'%';syncBusyVisual(busyProgressValue,'progress');return;}const s=Math.floor((Date.now()-busyStarted)/1000),soft=Math.min(9,Math.floor(s/4));busyElapsed.textContent=soft+'%';syncBusyVisual(soft,'indeterminate')},1000)}
...
function renderFourScriptAnalysis(x){
  ...
  const timelineHtml=`<section class="lc4-section"><h3>Kronologi Fakta dan Posisi Para Pihak</h3>${matrix(['Waktu','Peristiwa','Evidence Tag'],timeline.map(t=>[esc(t.time||'-'),esc(t.event||'-'),esc(t.evidence_tag||'-')]))}</section>`;
...
function startCaseProgressPolling(token){
  ...
  const tick=async()=>{if(stopped||inFlight)return;inFlight=true;try{const r=await fetch(API+'/case-analysis/progress/'+encodeURIComponent(token),{cache:'no-store'});if(!r.ok)return;const d=await r.json(),p=d.data||{};setBusyProgress(p.percent||0,caseProgressLabel(p.stage),p.detail||'Pembacaan dan analisis sedang berlangsung.');if(p.stage==='UPLOAD_STORED'){caseFileUploadStored=true;setCaseUploadStatus('uploaded');}}catch(_){}finally{inFlight=false;}};
