import { authorityAnchorsForContext, officialQueriesForContext } from './legalOntology';
...
export interface OfficialLawCandidate {
  provider: 'JDIH_BPK'|'JDIHN'|'LOCAL_CORPUS';
  query: string;
  title: string;
  url: string;
  ...
  query_kind?: 'EXACT_CITATION'|'TOPICAL_DISCOVERY'|'USER_EXACT_CITATION'|'USER_DIRECT_URL'|'LOCAL_CORPUS';
...
async function fetchText(url: string, timeoutMs = 9000): Promise<{ ok: boolean; status: number; text: string; finalUrl: string; error?: string }> {
  const ctl = new AbortController(); const timer = setTimeout(() => ctl.abort(), timeoutMs);
  try {
    const r = await fetch(url,{signal:ctl.signal,redirect:'follow',headers:{'User-Agent':'LexiCore-Official-Law-Retriever/1.1','Accept':'text/html,application/xhtml+xml'}});
    return {ok:r.ok,status:r.status,text:await r.text(),finalUrl:r.url||url};
  }
  catch (err: any) {
    const name=cleanText(err?.name||'FetchError');
    const message=cleanText(err?.message||String(err||''));
    return {ok:false,status:0,text:'',finalUrl:url,error:`${name}${message?`: ${message}`:''}`};
...
interface WebDiscoveryHit { title:string; url:string; locator:string; }
...
async function resolveCanonicalSeeds(
  ...
  for(const seed of seeds.slice(0,CANONICAL_SEED_MAX)){
    ...
    for(const r of result.resolved){
      candidates.push({
        provider: r.provider==='JDIHN' ? 'JDIHN' : 'JDIH_BPK',
        query,
        title: r.title,
        url: r.url,
...
export function buildOfficialLawQueries(input: {
  ...
  const queries = [...quoted, ...ontologyQueries, titleQuery].filter(x => x.length >= 6);
...
async function discoverAutoOfficialLaw(input: { mode: RegulatoryMode; queries: string[]; tempusYear?: number; maxCandidates?: number; domain?: string; caseText?: string }): Promise<OfficialLawRetrievalResult> {
  ...
  for(const query of input.queries.slice(0,8)){
  ...
  const queryOrder=input.queries.slice(0,8).filter(q=>(byQuery.get(q)?.length||0)>0);
  ...
  for(const item of selected){
    const detail=await fetchText(item.url,9000);
    if(!detail.ok){
    ...
    if(!identityVerified){
      const topical=topicalCandidateAccepts(item.query,input.domain,title,fullText,id.instrument_type,input.caseText);
      if(!topical.ok){
  ...
  for(const c of candidates){
    const fam=instrumentFamily(c.instrument_type)||'UNKNOWN';
    const key=`${fam}|${normalize(String(c.number||''))}|${c.year||0}|${normalize(c.title)}`;
    const current=canonical.get(key);
    if(!current || (c.material_nexus_score||0)>(current.material_nexus_score||0)) canonical.set(key,c);
  }
  const finalCandidates=[...canonical.values()].slice(0,maxCandidates);

  const searchAttempts=Math.min(8,input.queries.length);
  ...
  const providerStatus = !reachableSearches
    ? 'UNREACHABLE'
    : !found.length
      ? 'REACHABLE_NO_LINKS'
  ...
  const providerNote = providerStatus==='UNREACHABLE'
    ? 'JDIH BPK tidak dapat dijangkau pada seluruh percobaan pencarian.'
    : providerStatus==='REACHABLE_NO_LINKS'
      ? 'Endpoint pencarian JDIH BPK dapat dijangkau, tetapi parser tidak menemukan link detail kandidat.'
      : providerStatus==='REACHABLE_LINKS_REJECTED'
        ? 'Link detail JDIH BPK ditemukan, tetapi seluruh kandidat gugur pada pemeriksaan identity/material-nexus.'
        : 'Kandidat sumber resmi JDIH BPK ditemukan dan lolos pemeriksaan awal.';

  return {
    mode:input.mode,
    queries:input.queries,
    candidates:finalCandidates,
    providers:[{name:'JDIH_BPK',status:providerStatus,query_count:input.queries.length}],
    notes:[providerNote,'Strict identity guard aktif. Exact citation wajib cocok family/authority/number/year; topical discovery wajib lolos hierarchy + material-nexus + independent concept support. Query lexical fallback hanya dipakai bila ontology tidak menghasilkan issue-driven query.'],
    diagnostics,
  };
}
...
function inferInstrumentIdentityFromOfficialUrl(raw:string):{
  ...
  try{
    ...
    const relaxed=basename.match(
      /\b(UNDANG[- ]UNDANG|UU|PERPPU|PERATURAN\s+PEMERINTAH|PP|PERATURAN\s+PRESIDEN|PERPRES|PERATURAN\s+DAERAH|PERDA|PERATURAN\s+BUPATI|PERBUP|PERATURAN\s+WALI(?:KOTA)?|PERWALI|PERATURAN\s+MENTERI|PERMEN)\b[\s.-]*(?:(?:NO|NOMOR)\.?\s*)?([0-9A-Za-z./-]+)\s*(?:TH|TAHUN)\.?\s*(\d{4})\b/i
    );
    if(!relaxed) return {};
    ...
    const label=family==='UU'?'Undang-Undang':
      ...
      family==='PERMEN'?'Peraturan Menteri':rawType;
...
function manualCandidateFromOfficialPdfUrl(
  ...
  return {
    provider:host==='jdihn.go.id'||host.endsWith('.jdihn.go.id')?'JDIHN':'JDIH_BPK',
    query:rawInput,
...
export async function discoverOfficialLaw(input: {
  ...
  const manual=manualEnabled?await resolveManualAuthorities(input.manualAuthorities,input.tempusYear):{candidates:[],resolutions:[]};
  const auto=autoEnabled
    ? await discoverAutoOfficialLaw({mode:input.mode,queries:input.queries,tempusYear:input.tempusYear,maxCandidates:input.maxCandidates,domain:input.domain,caseText:input.caseText})
    : {mode:input.mode,queries:[] as string[],candidates:[] as OfficialLawCandidate[],providers:[{name:'JDIH_BPK',status:'MANUAL_SCOPE',query_count:0}],notes:['Auto-discovery dinonaktifkan oleh strategi sumber manual.']} as OfficialLawRetrievalResult;
  ...
  const providerStatus=finalCandidates.length?'REACHABLE_CANDIDATES':(manualEnabled&&manual.resolutions.length&&!autoEnabled?'MANUAL_NO_VERIFIED_SOURCE':auto.providers?.[0]?.status||'REACHABLE_NO_CANDIDATE');
  ...
  return {
    ...
    candidates:finalCandidates,
    providers:[{name:'JDIH_BPK',status:providerStatus,query_count:(auto.queries||[]).length}],
